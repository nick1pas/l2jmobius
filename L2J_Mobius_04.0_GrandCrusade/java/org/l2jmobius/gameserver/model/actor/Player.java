...
public class Player extends Playable
{
	...
	public void refreshExpertisePenalty()
	{
		...
		int armorPenalty = 0;
		int weaponPenalty = 0;
		for (Item item : _inventory.getPaperdollItems(item -> (item != null) && ((item.getItemType() != EtcItemType.ARROW) && (item.getItemType() != EtcItemType.BOLT)) && item.getItem().getCrystalType().isGreater(expertiseLevel)))
		{
			if (item.isArmor())
			{
